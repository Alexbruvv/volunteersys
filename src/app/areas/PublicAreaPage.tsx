...
export default function PublicAreaPage({
    ...
    return (
        <div className="p-4">
            ...
            {area.schedule.slots.map((slot) => slot.scheduleBlock)
            .filter((value, index, array) => array.findIndex(v => v.id === value.id) === index).map((block) => (
            <Fragment>
                ...
                <table className="table is-fullwidth is-bordered is-hoverable is-striped">
                    <thead>
                        <tr>
                            <th style="vertical-align: bottom" className="has-text-centered">Role</th>
                            {area.schedule.slots.filter(slot => slot.scheduleBlockId === block.id).map((slot) => (
